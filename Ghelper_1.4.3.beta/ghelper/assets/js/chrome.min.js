manifest=get_manifest();var sync_session_id=false;var trytime=10;var api_urls=[];api_urls.push('https://www.chromehelper.net');api_urls.push('https://www.speedstunnel.com');api_urls.push('https://www.ghelper.org');api_urls.push('https://ipv6.chromehelper.net');api_urls.push('https://cdn.speedstunnel.com');api_urls.push('https://global.speedstunnel.com');function notify(title,options={}){return new Notification(title,options);}
...
function lang(name){return chrome.i18n.getMessage(name);}
...
if(data.headers.open_tab){chrome.tabs.create(data.headers.open_tab,function(){});}
if(data.headers.location){window.location=data.headers.location;}}
if(data.html){for(tag in data.html){$(tag).html(data.html[tag]);}
handle_html();}}
function handle_html(){$('form').submit(function(){var url=$(this).attr('action');var data=$(this).serialize();request(url,'POST',data,false);return false;});$('.ajax').click(function(){var url=$(this).attr('href');request(url,'GET',null,false);return false;});}
function session(action='reload',async=true,up=false,tn=1){var data=false;if(tn>=trytime){return data;}
var pdata=get_local_data();var url=purl('/server/session',up);pdata['action']=action;var conf={};conf['url']=url;conf['type']='POST';conf['async']=async;conf['cache']=false;conf['timeout']=10000;conf['data']=pdata;conf['success']=function(res){handle(res);data=res;}
...
$.ajax(conf);}
function start(){set_proxy_pac_key('');set_api_url('');if(session('start',false)){}else{notify('启动失败.');set_sync_time(30000);}}
function error_html(err){var html='<div class="container">';html+='<div class="alert alert-danger error"  role="alert">';html+='<h1><i class="glyphicon glyphicon-warning-sign"></i></h1>';html+='<h3>'+lang('error_code')+' : '+err.status+'</h3>';html+='<h4>'+lang('error_network')+'</h4>';html+='<h5><a href="error.html" target="_blank" class="btn btn-warning">'+lang('error_button')+'</a></h5>';html+='</div></div>';$('body').html(html);}
